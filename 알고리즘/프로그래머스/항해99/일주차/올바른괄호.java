...
public class 올바른괄호 {
    ...
    private boolean solution(String s) {
        ...
        if (s.length() % 2 != 0 || s.charAt(s.length() - 1) != ')') {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == '(') { // (,) 둘 중 하나만 들어옴
                queue.add(s.charAt(i));  //add, 와 offer , remove 과 poll는 실패시 false냐 오류냐 (add,remove을 쓰자 안정성)

            } else // 시작부터 닫는 괄호면 ? poll 했을 때 뺼 수 있는 게 없기에 에러가 날껀데
                queue.remove(); // 시작을 미리 걸렀기에 )부터 실행될 일은 없다.
        }
